...
## Ordering rationale — answer trust questions in sequence
This ordering is not just tidy presentation.
It matches the trust questions a reader naturally asks:
1. **Who is this?** → identity anchor
2. **Who authorizes this surface?** → capability authority
3. **What evidence is declared?** → linked proof material
4. **What did we observe right now?** → live observations
5. **What are we only inferring?** → inferred claims / caveats

That sequencing matters because confidence is built cumulatively.
Identity and authority should frame the later sections; live observations should not overwrite durable identity facts; and inference should appear last so it is not mistaken for stronger evidence than it is.
