...
Use this as the copy-paste pack for the actual submission form, aligned to current product truth.

## Project name
**ÆNS — ENS Root Explorer + Write Records**

## One-line pitch
ÆNS gives agents and operators two clean ENS-native surfaces: inspect a root identity and write its `aens.*` records from a wallet.
...
## Problem statement
Most ENS demos for agents either overreach into vague “agent platforms” or bury the actual useful action behind too many speculative routes. The useful core is smaller: see the current ENS truth for a root identity, then write the records you need without ceremony.

## Solution / long description
ÆNS now does exactly two things:
...
The app intentionally does **not** pretend there is a separate research/discovery product surface anymore. The current product truth is the explorer plus the write flow.
...
- The public deployment exposes only the two real surfaces:
  - `https://aens-nine.vercel.app/`
  - `https://aens-nine.vercel.app/write-records/`
- Legacy extra surfaces are retired and redirected away.
